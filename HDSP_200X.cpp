...
void HDSP_200X::displayUpdate(void) {
    for (uint8_t i = 0; i < 4 * len; i++) { // shift down (by ' ', 0x20) to match actual matrix map from chars (ignore control chars)
        uint8_t charOffset = (currentString[i] - ' ');
        for (uint8_t j = 0; j < 5; j++) {
            chars[i][j] = pgm_read_byte_near(&char_data[charOffset][j]);
            // read from map, subtract ' ' to get to visible characters
            // read from program memory
        }
    }
    for (uint8_t i = 0; i < 5; i++) {
        for (uint8_t j = (num - 1); j >= 0; j--) { // write last 4 first, then write the displays used
